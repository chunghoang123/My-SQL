...
-- Hiển thị loại phòng và giá thuê theo ngày, sắp xếp theo giá tăng dầ
select room_type, rooms.price_per_day
from rooms
group by price_per_day asc;
...
from guests g
left join bookings b on g.guest_id = g.guest_id
group by g.guest_id;
...
select g.guest_name, count(b.booking_id) as total_bookings
from guests g
join bookings b on g.guest_id = g.guest_id
group by g.guest_id
